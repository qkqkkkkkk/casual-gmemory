...
class PDDLRecorder(BaseRecorder): 
    def __post_init__(self):
        ...
        # ← 新增：轨迹相关统计
        self.aspect_coverages: list[int] = []    # 每个 task 覆盖的 aspect 种类数
        self.redundant_rates:  list[float] = []  # 每个 task 的重复修改率
    ...
    #新增 trace 参数
    def task_end(self, reward: float, done: bool, trace: list[dict] = None):
        game_name: str = self.current_task_config.get('game_name') 
        if game_name is None:
            raise ValueError('The task should have an attribute: `game`.')
        ...
        # ← 新增：计算轨迹指标
        if trace:
            aspects = [s["aspect"] for s in trace]
            unique_aspects = set(aspects)
            
            # 指标1：覆盖了多少种不同的 aspect
            self.aspect_coverages.append(len(unique_aspects))
            
            # 指标2：重复率 = 重复出现的 aspect 步骤数 / 总步骤数
            redundant = len(aspects) - len(unique_aspects)
            rate = redundant / len(aspects) if aspects else 0
            self.redundant_rates.append(rate)

        # ← 改这里：先算好字符串，再放进 f-string
        coverage_str = str(self.aspect_coverages[-1]) if self.aspect_coverages else 'N/A'
        redundant_str = f"{self.redundant_rates[-1]:.2f}" if self.redundant_rates else 'N/A'
        #message = f'reward: {reward}, done: {done}.\nave reward: {self._get_average_reward()}, ave done: {self._get_average_done()}'
        message = (
            f"reward: {reward}, done: {done}\n"
            f"ave reward: {self._get_average_reward():.3f}, "
            f"ave done: {self._get_average_done():.3f}\n"
            f"aspect_coverage: {coverage_str}, "
            f"redundant_rate: {redundant_str}"
        )
        self.log(message)
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "avg_done":            self._get_average_done(),
            "avg_aspect_coverage": np.mean(self.aspect_coverages) if self.aspect_coverages else 0,
            "avg_redundant_rate":  np.mean(self.redundant_rates)  if self.redundant_rates  else 0,
        }
