...
class MacNet(MetaMAS):
    ...
    def build_system(self, reasoning: ReasoningBase, mas_memory: MASMemoryBase, env: Env, config: dict):
        ...
        self._use_trace: bool = config.get('use_trace', False)  # ← 新增一行
        self._use_pddl_trace: bool = config.get('use_pddl_trace', False)  # ← 新增
    ...
    def schedule(self, task_config: dict) -> tuple[float, bool]:
        ...
        for i in range(env.max_trials):
            ...
            while zero_in_degree_queue:  
                ...
                action: str = ''   # ← 新增，防止三次全部失败时 action 未定义
                tries = 0
                while tries < 3:
                    try:
                        #修改
                        action: str = curr_node.execute(user_message, use_critic=self._use_critic, use_trace=self._use_trace, use_pddl_trace=self._use_pddl_trace)     
                        if action == '':
                            continue  
        ...
        # ↓ 新增：收集 sink node 的 trace，清空避免跨任务污染
        self._last_trace = self._collect_sink_trace() if self._use_trace else None
        for node in self._agent_nodes.values():
            node._refinement_trace = []
    ...
    def _collect_sink_trace(self) -> list[dict]:
        """收集所有 sink node（无空间后继）的 trace，合并去重"""
        merged, seen = [], set()
        for node in self._agent_nodes.values():
            # sink node = spatial_successors 为空（decision node 已经 disconnect）
            if len(node.spatial_successors) == 0:
                for step in node._refinement_trace:
