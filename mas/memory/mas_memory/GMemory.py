...
class GMemory(MASMemoryBase):
    ...
    def __post_init__(self):
        super().__post_init__()
        # Causal comparisons must read the same persisted memory snapshot in both
        # branches.  This flag disables every cross-trial write below.
        self._read_only: bool = bool(self.global_config.get('read_only', False))
...
class TaskLayer:
    ...
    def cluster_tasks(self) -> None:
        ...
        try: 
            # 直接调用 FINCH 函数，传入数据 X，并使用正确的参数名 distance
            c, num_clust, req_c = FINCH(data=X, distance='cosine')
            
            # FINCH 返回的 c 是一个二维数组，包含不同层级的聚类结果。
            # c[:, 0] 代表第一层（最基础/最细粒度）的聚类标签，完美替代 fit_predict
            labels = c[:, 0]
