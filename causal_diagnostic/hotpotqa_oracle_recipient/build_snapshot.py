#!/usr/bin/env python3
"""Build one frozen native-GMemory snapshot from registered HotpotQA data."""
...
from causal_diagnostic.hotpotqa_oracle_recipient.provenance import SNAPSHOT_SCHEMA
...
from causal_diagnostic.hotpotqa_oracle_recipient.utils import seed_everything
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--support-count", type=int, default=50)
    parser.add_argument("--evaluation-count", type=int, default=100)
...
def _validate_args(args: argparse.Namespace) -> None:
    if args.support_count < 1 or args.evaluation_count < 1:
        raise SystemExit("support/evaluation counts must be at least 1")
    if args.node_num < 2:
        raise SystemExit("--node-num must be at least 2")
    if args.max_trials != 1:
        raise SystemExit("offline HotpotQA requires --max-trials 1")
...
def main(argv: Sequence[str] | None = None) -> Path:
    ...
    actual_prefix = [int(row["task_id"]) for row in records]
    ...
    progress_bar = tqdm(
        ...
        desc="Building HotpotQA support memory",
        unit="question",
        dynamic_ncols=True,
        mininterval=2.0,
    )
    ...
    try:
        for index in progress_bar:
            ...
            if after != before + 1:
                raise RuntimeError(
                    f"GMemory added {after - before} records for one support question"
                )
            record = {
                "task_id": current_claim_id,
            ...
            progress_bar.set_postfix_str(
                f"question {task['id']}, exact_match={bool(done)}", refresh=True
            )
            _write_progress(
                progress_path,
                status="running",
