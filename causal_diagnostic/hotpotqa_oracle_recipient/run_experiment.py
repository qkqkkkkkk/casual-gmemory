#!/usr/bin/env python3
"""Run isolated native-GMemory RQ2/RQ3/RQ4 interventions on HotpotQA."""
...
from causal_diagnostic.hotpotqa_oracle_recipient.offline_env import OfflineHotpotQAEnv
from causal_diagnostic.hotpotqa_oracle_recipient.metrics import score_supporting_facts
from causal_diagnostic.hotpotqa_oracle_recipient.provenance import (
    SnapshotProvenanceError,
    reconcile_retest_design,
    validate_snapshot_manifest,
)
from causal_diagnostic.hotpotqa_oracle_recipient.sampling import (
    configure_hotpotqa_sampling,
)
from causal_diagnostic.hotpotqa_oracle_recipient.prompts import (
    HOTPOTQA_SYSTEM_PROMPT,
    prepare_example,
)
from causal_diagnostic.hotpotqa_oracle_recipient.utils import (
    candidate_metadata,
    memory_config,
    seed_everything,
)
...
RUNNER_SCHEMA = "native-gmemory-macnet-hotpotqa-rq234-v1"
...
def _write_gate_training_summary(
    ...
    for conditions in grouped.values():
        ...
        if not use_rows or not drop_rows:
            continue
        q_use = sum(float(row["outcome"]["team_score"]) for row in use_rows) / len(
            use_rows
        )
        q_drop = sum(
            float(row["outcome"]["team_score"]) for row in drop_rows
        ) / len(drop_rows)
    ...
    path.write_text(
        json.dumps(
            {
                "schema": "gmemory-hotpotqa-gate-training-collection-v1",
...
def _run_branch(
    ...
    *,
    ...
    env = OfflineHotpotQAEnv(max_trials=1)
    ...
    configure_hotpotqa_sampling(mas, args.temperature)
    for agent in mas.agents_team.values():
        agent.add_task_instruction(HOTPOTQA_SYSTEM_PROMPT)
    mas._decision_node._agent.add_task_instruction(HOTPOTQA_SYSTEM_PROMPT)
    ...
    outcome = {
        ...
        "score_definition": "official_hotpotqa_normalized_answer_token_f1",
        "prediction": env.last_prediction,
        "gold_answer": branch_task["answer"],
        "answer_metrics": env.last_metrics,
        "memory_records_before": before,
        "memory_records_after": after,
    }
    if not args.gate_training_only:
        decision_evidence_metric = score_supporting_facts(
            mas.execution_trace[-1]["decision"]["raw_output"],
            branch_task.get("gold_evidence_page_sets", []),
        )
        outcome.update(
            local_metrics={
                name: score_supporting_facts(
...
def main(argv: Sequence[str] | None = None) -> Path:
    ...
    examples = load_hotpotqa(args.test)
    ...
    progress = tqdm(
        ...
        desc=f"HotpotQA seed {args.sample_seed_base}",
        unit="branch",
        dynamic_ncols=True,
        mininterval=1.0,
    )
            ...
            for repeat_index in range(args.repeats):
                ...
                for condition in conditions:
                    ...
                    row = {
                        ...
                        "run_hash": run_hash,
                        "event_id": (
                            f"hotpotqa-{claim_id}-{candidate['candidate_id']}"
                        ),
                        "task_id": claim_id,
                        "task": {
                            "task_id": claim_id,
    ...
    if not existing:
        raise SystemExit("no complete HotpotQA branches were collected")
    _update_progress(
        status="analyzing",
        resolved_branches=resolved_branches,
        persisted_branches=len(existing),
