#!/usr/bin/env python3
"""Run and resume the isolated offline-HotpotQA RQ2/RQ3/RQ4 pipeline."""
...
ORCHESTRATOR_SCHEMA = "native-gmemory-macnet-hotpotqa-pipeline-v1"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=Path(
            ...
            "hotpotqa_support50_7b_v1/g-memory"
        ),
    )
    parser.add_argument(
        "--results-root",
        ...
        default=Path(
            ...
            "native_hotpotqa_rq234_pilot_7b_v1"
        ),
    )
...
def _validate_args(args: argparse.Namespace) -> None:
    if args.support_count < 1 or args.evaluation_count < 1:
        raise SystemExit("--support-count and --evaluation-count must be at least 1")
...
def main(argv: Sequence[str] | None = None) -> Path:
    ...
    snapshot_command = [
        sys.executable,
        "-m",
        "causal_diagnostic.hotpotqa_oracle_recipient.build_snapshot",
        "--data",
        str(args.data),
        "--support-count",
        str(args.support_count),
        "--evaluation-count",
    ...
    overall = tqdm(
        total=len(stages),
        desc="Complete HotpotQA experiment",
        unit="stage",
        dynamic_ncols=True,
    )
